...
const smtpTransportersByMailer = new Map()
const mailtrapClientsByCacheKey = new Map()
const resendClientsByMailer = new Map()
...
module.exports = {
  ...
  fn: async function ({
    ...
    const emailTemplateLayout = layout
      ? path.relative(
    ...
    switch (sails.config.mail.mailers[mailer]?.transport) {
      case 'smtp':
        ...
        const transporter = getSmtpTransporter(mailer, () =>
          nodemailer.createTransport({ ...smtpTransportConfig })
        )
        const smtpMail = {
          from: {
            name: fromName,
      ...
      case 'mailtrap':
        ...
        const mailtrapInfo = testInboxId
          ? await mailtrap.testing.send({ ...mailtrapMail })
          : await mailtrap.send({ ...mailtrapMail })
      ...
      case 'resend':
        ...
        const resend = getResendClient(mailer, () => new Resend(apiKey))
        const resendMail = {
...
function getSmtpTransporter(mailer, createTransporter) {
  if (!smtpTransportersByMailer.has(mailer)) {
    smtpTransportersByMailer.set(mailer, createTransporter())
  }

  return smtpTransportersByMailer.get(mailer)
}

function getMailtrapClient(mailer, testInboxId, createClient) {
  const cacheKey = `${mailer}:${testInboxId || ''}`

  if (!mailtrapClientsByCacheKey.has(cacheKey)) {
    mailtrapClientsByCacheKey.set(cacheKey, createClient())
  }

  return mailtrapClientsByCacheKey.get(cacheKey)
}

function getResendClient(mailer, createClient) {
  if (!resendClientsByMailer.has(mailer)) {
    resendClientsByMailer.set(mailer, createClient())
  }

  return resendClientsByMailer.get(mailer)
}
